  ...
  Scene(controller)
{
  ...
  mobSelectionId = getController().GetMobPackagePartitioner().GetPartition(Game::LocalPartition).FirstValidPackage();
...
void SelectMobScene::onUpdate(double elapsed) {
  ...
  if (!gotoNextScene) {
    if (Input().Has(InputEvents::pressed_ui_left)) {
      ...
      if (selectInputCooldown <= 0) {
        // Go to previous mob 
        selectInputCooldown = maxSelectInputCooldown;
        mobSelectionId = getController().GetMobPackagePartitioner().GetPartition(Game::LocalPartition).GetPackageBefore(mobSelectionId);
  ...
  auto& mobinfo = getController().GetMobPackagePartitioner().GetPartition(Game::LocalPartition).FindPackageByID(mobSelectionId);
  ...
  if (Input().Has(InputEvents::pressed_confirm) && !gotoNextScene) {
    ...
    MobPackageManager& packageManager = getController().GetMobPackagePartitioner().GetPartition(Game::LocalPartition);
    ...
    if (!mob) {
      ...
      auto& meta = getController().GetPlayerPackagePartitioner().GetPartition(Game::LocalPartition).FindPackageByID(selectedNaviId);
